...
case "$TOOL" in
  ...
  Bash)
    ...
    if printf '%s' "$CMD" | grep -Eq '\bgit\s+(add|commit)\b'; then
      ...
      #    Exclude this guard script itself — it necessarily contains the very
      #    pattern strings it looks for, and would otherwise block its own commit.
      if git rev-parse --git-dir >/dev/null 2>&1; then
        STAGED=$(git diff --cached --no-color -- . ':(exclude).claude/hooks/guard-secrets.sh' 2>/dev/null || true)
